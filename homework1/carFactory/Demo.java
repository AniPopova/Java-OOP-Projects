package homework1.carFactory;

import static accessControl.carFactory.TypeExtra.NAVIGATION;
...
int counter = 0;
    ...
    for (int i = 0; i < numberOfCars; i++) {
      if (counter % 2 != 0) {
        ...
        cars.stream().filter((Predicate<? super Car>) extras);
      } else {
        cars.stream().filter(Predicate.isEqual(NAVIGATION)) ;
      }
